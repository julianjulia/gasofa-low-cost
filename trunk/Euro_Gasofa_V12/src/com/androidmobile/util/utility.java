...
public class utility {
	...
	public static void tratamientoDatosGasolinera(final DatosIni datos,final Context mContext)
	  ...
	  final WebView webview = (WebView)  ((Activity) mContext).findViewById(R.id.mainWebView);
		class taskgasNew extends AsyncTask<Void, Void, String>  implements OnCancelListener{
			...
			if (mun.equals("HOSPITALET DE LLOBREGAT (L)"))
				...
				ArrayList<Gasolinera> alg = new ArrayList<Gasolinera>();
								...
								//	while ((text = brNew.readLine()).trim() != null) {
									...
									while (pos != -1 ) {//añadido 13/09/2013
										...
										Gasolinera g = new Gasolinera(UPV.trim(),fecha.trim(), nombre.trim(),
												...
												gasolina98.trim(), gasoleo.trim(),"false");
										alg.add(g);
					...
					((Activity) mContext).runOnUiThread(new Runnable() {
				        ...
				        public void run() {
				        	...
				        	//prueba visualizacion resultados 27/03/2014
			 ...
			 String json = EMPTY_GSOLIN_LIST ;
				...
				String id_c="";
				// comprobar que existe alerta//
				for (Gasolinera g: algOrder){
					if (!g.getGasoleo().equals("-"))
						id_c="4";
					else if (!g.getGasolina95().equals("-"))
						id_c="1";
					else if (!g.getGasolina98().equals("-"))
						id_c="3";
					if (bdgas.buscarAlerta(g.getUPV(),id_c)!= null){
						  g.setRunflag("checked");
